...
def get_ids():
    ...
    ids = [row['bioguideid'] for row in resp.json() if (row.get('edited') and row.get('party') == 'R' and row.get('voting_member') == 'TRUE' and row.get('position') == 'Opposed') and row.get('chamber') == 'house']
    return sorted(ids)
...
def make_promotion_thumb():
    ...
    cropped = cropped_thumbnail(image, [2000 * ASPECT_RATIO, 2000])
